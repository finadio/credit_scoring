<x-app-layout>
    <x-slot name="header">
        <h2 class="h4 mb-0">
            {{ __('Edit Parameter Scoring') }} - {{ $parameter->parameter_name }}
        </h2>
    </x-slot>
    ...
    <div class="py-4">
        <div class="container">
            <div class="card shadow-sm">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.parameters.update', $parameter->id) }}" x-data="{ rulesType: '{{ old('rules_type', $rules_type) }}', options: @json(old('options', $options)) }">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <x-text-input id="parameter_name" class="form-control @error('parameter_name') is-invalid @enderror" type="text" name="parameter_name" :value="old('parameter_name', $parameter->parameter_name)" required autofocus />
                        ...
                        <div class="mb-3">
                            ...
                            <select id="category" name="category" class="form-select @error('category') is-invalid @enderror" required>
                                ...
                                @foreach($categories as $category)
                                    <option value="{{ $category }}" {{ old('category', $parameter->category) == $category ? 'selected' : '' }}>{{ $category }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="description" name="description" rows="3" class="form-control @error('description') is-invalid @enderror">{{ old('description', $parameter->description) }}</textarea>
